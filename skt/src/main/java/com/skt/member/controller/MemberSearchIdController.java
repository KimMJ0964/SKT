...
public class MemberSearchIdController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    String input = request.getParameter("input"); // 이메일 또는 전화번호가 들어올 변수
	    
	    System.out.println("입력 값: " + input); // 입력 값 확인을 위한 로그
	    
	    MemberService memberService = new MemberServiceImpl();
	    Member searchId = memberService.searchByEmailOrPhone(input);
	    
	    if (searchId != null) {
	        request.getSession().setAttribute("searchId", searchId.getMemId());
